...
- `docs/v1/decisions.md` — owner/curation rulings, grouped by area. Read the group
  for the area you are about to touch.
- `docs/v1/progress-archive.md` — log entries older than the five kept below.
...
language for non-engineers in `docs/v1/owner-tasks/` — one document per task,
...
| 2 | products.json freshness owner | **closed** 2026-09-10 — owner ruled: exports arrive **ad hoc** when updates are known, no schedule; at each drop engineering runs the recorded chain (diff → aliases → stage2 → stage4 → index → ping, in `docs/v1/decisions.md` §5). First drop processed same day: dotBAR flavors 1470/1471 → one dotBAR family, index 4,002 → 4,006, live-verified |
...
`docs/v1/progress-archive.md`. Detail belongs in the commit, the code, or the
...
the newest five to the top of `docs/v1/progress-archive.md`, so this file's length
...
Owner rulings that constrain future work go to `docs/v1/decisions.md` — not here,
and not twice.

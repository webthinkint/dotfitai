...
| v1 runtime (§11) | §11 | **built** — `DotFit.Agents` library + `dotfit-agent` CLI, 64 tests green; live smoke: embeddings + both chat deployments verified (real guardrail verdicts, fail-open path exercised); end-to-end `ask`/`search` blocked by a **service-side failure on `dotfitsearch`** (portal Search Explorer 503s too; Azure status all green → instance-specific); remaining: finish the ask smoke when the service recovers | `runtime/` |
...
| 9 | Runtime live smoke | **blocked on the search service itself** (2026-09-07 evening): `dotfitsearch` accepts TLS but drops *authenticated* queries — curl + the .NET SDK, api-versions 2024-07-01 and 2026-04-01, request sent, zero bytes back — while unauthenticated probes get an instant 401; portal Search Explorer 503s; Azure status all green → instance-specific (serverless autoscale/network-intent suspected; the earlier IP-allowlist vs all-networks modes behaved identically, so it was never the ACLs). Embeddings + chat deployments on the same account work. Owner: watch the service (Overview status / Activity Log), support ticket if it persists. On recovery: `dotfit-agent search "creatine loading" --trace` then `dotfit-agent ask "…" --trace` |

...
| v1 runtime (§11) | §11 | **built + live smoke run** — `DotFit.Agents` library + `dotfit-agent` CLI, 64 tests green; end-to-end `search`/`ask` verified 2026-09-07 against `kb-main-v2` (guardrail → rewrite → aliases → hybrid search → grounded streamed answer with `[n]` citations → post-check, all stages traced). `search` returns pdsrg/product/qa hits with authority re-rank; `ask` answers grounded and cites correctly. The `ask` run exits 1 on a **claims-language post-check FAIL** — the auditor flagged a passage the answer had *quoted verbatim from the corpus*, so triage whether the audit should exempt quoted source text (new work item, not an infra issue). Retrieval was blocked until 2026-09-07 by a wedged `kb-main` (open item 9) | `runtime/` |

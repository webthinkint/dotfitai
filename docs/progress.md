...
| v1 runtime (§11) | §11 | **built** — `DotFit.Agents` library + `dotfit-agent` CLI, 64 tests green; §11 pipeline component-for-component (guardrail → rewrite + alias expansion → hybrid search → grounded [n]-cited streamed answer → post-check + claims audit); escalations short-circuit to a templated refusal; live end-to-end smoke pending the Azure network ACLs (both services 403 the machine's current source IP — the Python pipeline 403s identically, so environment, not code) | `runtime/` |
...
| 9 | Runtime live smoke | **open** — both Azure services 403 the machine's current source IP (2026-09-07; Python 403s identically — network ACL, not code). Owner: add the current IP to the AI Search + Azure OpenAI Networking rules (or restore the VPN state). CLI commands are ready to rerun: `dotfit-agent ask \| search \| guardrail \| rewrite` |

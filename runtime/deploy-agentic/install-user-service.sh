#!/usr/bin/env bash
# runtime/deploy-agentic/install-user-service.sh — install dotfit-agentic-service
...
#
#   1. dotnet publish the SSE service to ~/.local/share/dotfit/agentic-service
...
UNIT_SRC="$SCRIPT_DIR/dotfit-agentic-service.service"
UNIT_DST="$HOME/.config/systemd/user/dotfit-agentic-service.service"
PUBLISH_DIR="$HOME/.local/share/dotfit/agentic-service"
BIND="${BIND:-0.0.0.0:5299}"
PORT="${BIND##*:}"
...
# --- 1. publish -----------------------------------------------------------
echo "publishing DotFit.Agentic.Service (Release)…"
dotnet publish "$REPO_ROOT/runtime/src/DotFit.Agentic.Service" \
...
if [ -f "$SCRIPT_DIR/turn-log" ]; then
    mkdir -p "$HOME/.local/bin"
    install -m 0755 "$SCRIPT_DIR/turn-log" "$HOME/.local/bin/dotfit-turn-log"
    echo "installed ~/.local/bin/dotfit-turn-log"
fi
...
systemctl --user enable --now dotfit-agentic-service.service
systemctl --user restart dotfit-agentic-service.service

# The boot validates the whole .env slice it needs (AgenticFactory.Needs), so
...
for _ in $(seq 1 30); do
    if curl -sf "http://127.0.0.1:$PORT/healthz" > /dev/null 2>&1; then
        ...
        systemctl --user status dotfit-agentic-service.service --no-pager | head -n 4
        exit 0
    fi
    if ! systemctl --user is-active --quiet dotfit-agentic-service.service; then
        echo "ERROR: the service exited — a fail-closed boot or a bad .env. Logs:" >&2
        journalctl --user -u dotfit-agentic-service.service -n 20 --no-pager >&2
        exit 1
    fi
    sleep 1
done
